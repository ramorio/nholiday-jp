...
class NHolidayJp {
  /**
   * Date型の日付が祝日ならその名称を取得
   * 祝日でなければnull
   * 
   * @param {Date} date 日付 
   */
  static getNameByDate(date) {
    console.log(date.getYear());
    return NHolidayJp.getName(date.getFullYear(), date.getMonth() + 1, date.getDate());
  }
   ...
   * @returns {string} 祝日名
   */
  static getName(year, month, date) {
    const holidays = NHolidayJp.getByYear(year);
    const holiday = holidays.find(v => v.month === month && v.date === date);
    if (holiday) {
      return holiday.name;
    }
    return null;
  ...
  static getByMonth(year, month) {
    const holidays = NHolidayJp.getByYear(year);
    return holidays.filter(v => v.month === month);
  }
  ...
  static getByYear(year) {
    ...
    if (!created.has(year)) {
      created.set(year, NHolidayJp.create(year));
    }
    return created.get(year).map(v => v.clone());
  }
...
class Holiday {
  ...
  clone() {
    return new Holiday(this.month, this.date, this.name);
  }
}
